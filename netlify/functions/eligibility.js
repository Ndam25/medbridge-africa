...
exports.handler = async (event) => {
  ...
  if (!url || !key) {
    return { statusCode:500, headers:{'content-type':'application/json'},
      body: JSON.stringify({ ok:false, error:'ENV_MISSING' }) };
  }
  ...
  if (error) {
    return { statusCode:500, headers:{'content-type':'application/json'},
      body: JSON.stringify({ ok:false, error:'DB_INSERT_FAILED', detail: error.message }) };
  }
  ...
  if (RESEND && TO && FROM) {
    try {
      ...
      return { statusCode:201, headers:{'content-type':'application/json'},
        body: JSON.stringify({ ok:true, status:'notified' }) };
    } catch {
      return { statusCode:201, headers:{'content-type':'application/json'},
        body: JSON.stringify({ ok:true, status:'stored_only' }) };
    }
